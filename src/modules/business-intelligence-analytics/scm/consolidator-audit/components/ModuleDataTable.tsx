...
export function ModuleDataTable<TData, TValue>({
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border dark:border-zinc-700 bg-card shadow-sm overflow-hidden">
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup: HeaderGroup<TData>) => (
              <TableRow key={headerGroup.id} className="bg-muted/10">
                {headerGroup.headers.map((header: Header<TData, unknown>) => {
                  return (
                    <TableHead key={header.id} className="font-extrabold py-4 px-4 border-r last:border-r-0 dark:border-zinc-700 text-center text-sm md:text-base lg:text-lg text-foreground">
              ...
              Array.from({ length: 5 }).map((_, rowIndex) => (
                <TableRow key={rowIndex} className="hover:bg-transparent">
                  {columns.map((_, colIndex) => (
                    <TableCell key={colIndex} className="py-3 border-r last:border-r-0 dark:border-zinc-700">
              ...
              table.getRowModel().rows.map((row: Row<TData>) => (
                ...
                >
                  {row.getVisibleCells().map((cell: Cell<TData, unknown>) => (
                    <TableCell key={cell.id} className="p-0 h-full align-stretch border-r last:border-r-0 dark:border-zinc-700">
                      {flexRender(
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex-1 text-sm text-muted-foreground font-medium">
          {table.getFilteredRowModel().rows.length === 0 ? "0" : `${table.getState().pagination.pageIndex * table.getState().pagination.pageSize + 1}-${Math.min((table.getState().pagination.pageIndex + 1) * table.getState().pagination.pageSize, table.getFilteredRowModel().rows.length)}`} of {table.getFilteredRowModel().rows.length} rows
        </div>
