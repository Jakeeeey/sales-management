...
import type { CustomerOrdersSummary } from "../types";
...
/* ─── Main Component ──────────────────────────────────────────── */
type Props = { customerSummaries: CustomerOrdersSummary[] };

export function CustomersTab({ customerSummaries }: Props) {
  ...
  const [sortKey, setSortKey] =
    React.useState<keyof CustomerOrdersSummary>("totalOrders");
  const [sortDir, setSortDir] = React.useState<"asc" | "desc">("desc");
  const [currentPage, setCurrentPage] = React.useState(1);
  const [itemsPerPage, setItemsPerPage] = React.useState(10);

  /* ─── Drill-down state ── */
  const [drillCustomer] = React.useState("all");
  ...
  const [modalCustomer, setModalCustomer] =
    React.useState<CustomerOrdersSummary | null>(null);

  const handleSort = React.useCallback(
    (key: keyof CustomerOrdersSummary) => {
      if (key === sortKey) {
  ...
  const sorted = React.useMemo(
    ...
    [customerSummaries, sortKey, sortDir],
  );
  ...
  const filteredSorted = React.useMemo(() => {
    let data = sorted;
    if (drillCustomer !== "all")
      data = data.filter((s) => s.customerName === drillCustomer);
    ...
    // Apply search (product/customer/category) if provided
    if (appliedSearch && appliedSearch.trim() !== "") {
      const q = appliedSearch.trim().toLowerCase();
      data = data.filter((s) => s.customerName.toLowerCase().includes(q));
    }
    // Exclude fully consolidated customers when toggle is off
  ...
  const sortIcon = (col: keyof CustomerOrdersSummary) => (
  ...
  return (
    <div className="space-y-4">
      {/* ── Chart Row 1: Orders by Customer ── */}
      <Card className="border-muted ">
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Orders by Customer</CardTitle>
          <CardDescription>
              ...
              >
                {top10ByOrders.map((e, i) => (
                  <Cell
                    ...
                    style={cellStyle("ord", e.customerName)}
                  />
                ))}
              </Bar>
            </BarChart>
      ...
      {/* ── Chart Row 2: For Consolidation Orders by Customer ── */}
      {top10ByPending.length > 0 && (
        <Card className="border-muted ">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">
              For Consolidation Orders by Customer
            </CardTitle>
            <CardDescription>
              Customers with the most orders for consolidation — click for
              insights
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            >
              ...
              >
                ...
                <Bar
                  ...
                  radius={[0, 4, 4, 0]}
                  fill="#f59e0b"
                  onMouseEnter={(d) => setHoveredBar(`pend::${d.customerName}`)}
                  onMouseLeave={() => setHoveredBar(null)}
                  onClick={(d) => openModal(d.customerName)}
                  cursor="pointer"
                >
                  {top10ByPending.map((e, i) => (
                    <Cell
                      ...
                      style={cellStyle("pend", e.customerName)}
                    />
                  ))}
                </Bar>
              </BarChart>
      ...
      {/* ── Chart Row 3: Ordered Qty by Customer ── */}
      <Card className="border-muted ">
        <CardHeader className="pb-2">
          <CardTitle className="text-base">
            Ordered Quantity by Customer
          </CardTitle>
          <CardDescription>
            Top 10 by quantity ordered — click for insights
          </CardDescription>
          ...
          >
            ...
            >
              ...
              <Bar
                ...
                radius={[0, 4, 4, 0]}
                onMouseEnter={(d) => setHoveredBar(`qty::${d.customerName}`)}
                onMouseLeave={() => setHoveredBar(null)}
                onClick={(d) => openModal(d.customerName)}
                cursor="pointer"
              >
                {top10ByQty.map((e, i) => (
                  <Cell
                    ...
                    style={cellStyle("quantity", e.customerName)}
                  />
                ))}
              </Bar>
            </BarChart>
        ...
        <CardHeader className="pb-2">
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">
                All Customers — {filteredSorted.length} of{" "}
            ...
            {/* <div className="flex flex-wrap items-center gap-2">
              <select
                ...
                value={drillCustomer}
                onChange={(e) => {
                  setDrillCustomer(e.target.value);
                  setCurrentPage(1);
                }}
              >
                <option value="all">All Customers</option>
                {customerSummaries.map((s) => (
                  <option key={s.customerName} value={s.customerName}>
                    {s.customerName}
                  </option>
                ))}
              </select>
              ...
              {(drillCustomer !== "all" || drillStatus !== "all") && (
                <button
                  className="text-xs text-muted-foreground hover:text-foreground underline"
                  onClick={() => {
                    setDrillCustomer("all");
                    setDrillStatus("all");
                    setCurrentPage(1);
                  }}
                >
      ...
      >
        <DialogContent className="max-w-md ">
          <DialogHeader>
            ...
            <DialogDescription>
              Customer consolidation insights
            </DialogDescription>
          </DialogHeader>
          {modalCustomer && (
            <div className="space-y-0.5 pt-1">
              <InsightStat
                label="Total Orders"
                value={numFmt(modalCustomer.totalOrders)}
              />
              <InsightStat
                label="Consolidated"
                value={numFmt(modalCustomer.totalConsolidated)}
                color="text-emerald-600 dark:text-emerald-400"
              />
              <InsightStat
                label="For Consolidation"
                value={numFmt(modalCustomer.pendingOrders)}
                color={
                  modalCustomer.pendingOrders > 0
              ...
              <InsightStat
                ...
                value={pctFmt(
                  modalCustomer.totalOrders > 0
                    ? (modalCustomer.totalConsolidated /
                        modalCustomer.totalOrders) *
                        100
                    : 0,
                )}
              />
              ...
              <InsightStat
                label="Net Amount"
                value={`₱${numFmt(modalCustomer.totalNetAmount)}`}
              />
              <InsightStat
                label="Share %"
