...
async function performLogin(): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.error(`[SCM Proxy – Aging & SLOB] Login failed with status: ${res.status}`);
      return null;
    }
    ...
    console.error("[SCM Proxy – Aging & SLOB] Auth error:", error);
  }
  return null;
}
...
export async function scmProxyRequest(endpoint: string) {
  ...
  if (!token) {
    console.log("[SCM Proxy – Aging & SLOB] Token missing, logging in...");
  ...
  if (response.status === 401 || response.status === 403) {
    console.log("[SCM Proxy – Aging & SLOB] Token expired or invalid, retrying login...");
    token = (await performLogin()) ?? undefined;
    if (token) {
