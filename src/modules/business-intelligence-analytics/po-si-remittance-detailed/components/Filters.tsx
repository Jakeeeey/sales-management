    ...
    <div className="flex flex-wrap gap-6 items-end w-full bg-card p-4 rounded-xl border shadow-sm">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {['all-time', 'yesterday', 'today', 'tomorrow', 'this-week', 'this-month', 'this-year', 'custom'].map(preset => (
            <Button
              key={preset}
              variant={datePreset === preset ? "default" : "secondary"}
                ...
                <span className="truncate">
                  {isAllSelected 
                    ...
                    : selectedSuppliers.length === 1 
                      ? suppliersList.find(s => s.id === selectedSuppliers[0])?.name || '1 Supplier selected'
