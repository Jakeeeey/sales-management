...
export default function ProcessTimeline({
  ...
  const rowsWithSegments = React.useMemo(() => {
    ...
    for (const [rowIndex, recRaw] of rows.entries()) {
      const rec = recRaw as LeadTimeRow;
      const po = rec.poNo;
      ...
      const events = {
        created: rec.createdDate ? new Date(rec.createdDate) : null,
        approved: rec.approvedDate ? new Date(rec.approvedDate) : null,
        dispatch: rec.dispatchedDate ? new Date(rec.dispatchedDate) : null,
        delivered: rec.deliveredDate ? new Date(rec.deliveredDate) : null,
      };

      const poDate = rec.purchasedDate ? new Date(rec.purchasedDate) : null;
      ...
      const approvalDays = getDaysDiff(rec.createdDate, rec.approvedDate);
      const fulfillmentDays = getDaysDiff(rec.approvedDate, rec.dispatchedDate);
      const deliveryDays = getDaysDiff(rec.dispatchedDate, rec.deliveredDate);

      const approvalStatus = rec.approvalStatus;
      const fulfillmentStatus = rec.fulfillmentStatus;
      const deliveryStatus = rec.deliveryStatus;
      ...
      if (!existing) {
        ...
        if (!existing.label) {
          existing.label = rec.customerName;
        }
      }
    }
