...
export function LeadTimeTrendChart({ rows }: Props) {
  const data = React.useMemo(() => {
    ...
    for (const r of rows) {
      const key = (r.purchasedDate || "").slice(0, 10) || "Unknown";
      ...
      const app = getDaysDiff(r.purchasedDate, r.approvedDate);
      if (typeof app === "number") {
        entry.approvalSum += app;
        entry.approvalCount += 1;
      }
      
      const disp = getDaysDiff(r.purchasedDate, r.dispatchedDate);
      if (typeof disp === "number") {
        entry.dispatchSum += disp;
        entry.dispatchCount += 1;
      }
      
      const deliv = getDaysDiff(r.purchasedDate, r.deliveredDate);
      if (typeof deliv === "number") {
        entry.deliveredSum += deliv;
