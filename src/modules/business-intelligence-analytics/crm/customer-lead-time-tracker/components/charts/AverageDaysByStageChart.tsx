...
export function AverageDaysByStageChart({ rows }: Props) {
  const data = React.useMemo<StageDatum[]>(() => {
    ...
    for (const r of rows) {
      const app = getDaysDiff(r.createdDate, r.approvedDate);
      const disp = getDaysDiff(r.approvedDate, r.dispatchedDate);
      const del = getDaysDiff(r.dispatchedDate, r.deliveredDate);
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <XAxis dataKey="stage" />
              <YAxis tickFormatter={(v: number) => `${Number(v).toFixed(1)}d`} />
              <ChartTooltip content={<AverageDaysByStageTooltip />} />
              <Bar dataKey="days" fill="#2563eb">
                {data.map((entry: StageDatum, i: number) => (
