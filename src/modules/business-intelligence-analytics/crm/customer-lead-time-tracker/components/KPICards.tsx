...
function getDaysDiff(start: string | null | undefined, end: string | null | undefined): number | null {
  if (!start || !end) return null;
  const d1 = new Date(start).getTime();
  const d2 = new Date(end).getTime();
  if (isNaN(d1) || isNaN(d2)) return null;
  return Math.max(0, Math.floor((d2 - d1) / (1000 * 3600 * 24)));
}

export default function KPICards({ rows, loading = false }: KPICardsProps) {
  const approvalDays = rows.map((r) => getDaysDiff(r.purchasedDate, r.approvedDate));
  const pickedConsoDays = rows.map((r) => getDaysDiff(r.purchasedDate, r.pickedConsoDate));
  const dispatchDays = rows.map((r) => getDaysDiff(r.purchasedDate, r.dispatchedDate));

  const kpis = [
    {
      title: "Approval",
      desc: "Approval days vs PO Date",
      value: mean(approvalDays),
      highest: max(approvalDays),
    },
    {
      title: "Picked/Conso",
      desc: "Picked/Conso days vs PO Date",
      value: mean(pickedConsoDays),
      highest: max(pickedConsoDays),
    },
    {
      title: "Dispatch",
      desc: "Dispatch days vs PO Date",
      value: mean(dispatchDays),
      highest: max(dispatchDays),
    },
  ];
      ...
      {kpis.map((k, i) => {
        ...
        return (
          <Card key={i} className={cardClass}>
            ...
            <CardContent className="px-4 py-4 flex items-end justify-between">
              <div>
                <div className={`text-2xl font-bold tabular-nums ${valueClass}`}>
                  {loading ? (
                    ...
                    formatNumber(k.value, "en-PH", 1) + " Days Avg"
                  )}
                </div>
              </div>
              <div className="text-right">
                <div className="text-xs text-muted-foreground uppercase tracking-wider font-semibold mb-1">Highest</div>
                <div className={`text-lg font-bold tabular-nums ${k.highest != null && k.highest > 3 ? "text-red-600 dark:text-red-400" : "text-muted-foreground"}`}>
                  {loading ? (
