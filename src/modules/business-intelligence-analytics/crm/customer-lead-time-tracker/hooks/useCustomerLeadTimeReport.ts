...
export function useLeadTimeReport() {
  const [filters, setFilters] =
    React.useState<LeadTimeFilters>(getDefaultFilters);
  const [rows, setRows] = React.useState<LeadTimeRow[]>([]);
  const [customers, setCustomers] = React.useState<{ customer_code: string; customer_name: string }[]>([]);
  ...
  React.useEffect(() => {
    ...
    import("../providers/fetchProvider").then(({ fetchLeadTimeCustomers }) => {
      fetchLeadTimeCustomers(controller.signal)
        .then((data) => {
          if (!cancelled) {
            setCustomers(data);
