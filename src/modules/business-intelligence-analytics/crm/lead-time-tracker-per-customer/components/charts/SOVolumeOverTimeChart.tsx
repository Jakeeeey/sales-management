...
export function SOVolumeOverTimeChart({ rows, filters }: Props) {
  const data = React.useMemo<VolumeDatum[]>(() => {
    const map = new Map<number, number>();

    // Aggregate actual SO counts by month using purchasedDate
    for (const r of rows) {
      const dateStr = r.purchasedDate || r.createdDate;
      if (!dateStr) continue;
    ...
    // Build full month sequence (no gaps)
    ...
    // Map into chart-ready structure
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Customer Order Frequency (Monthly)</CardTitle>
        <CardDescription>
          How often this customer places orders over time
        </CardDescription>
      </CardHeader>
...
export default SOVolumeOverTimeChart;
