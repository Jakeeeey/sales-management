...
export const fetchManagerialData = async (startDate: string, endDate: string): Promise<VSalesPerformanceDataDto[]> => {
    try {
        ...
        const url = `/api/bia/crm/target-setting-reports/divisional-channel/managerial-channel?${params.toString()}`;
...
export const fetchDynamicTargets = async (startDate: string, endDate: string, divisionId?: number): Promise<Record<string, unknown>> => {
    try {
        ...
        const url = `/api/bia/crm/target-setting-reports/divisional-channel/managerial-channel/targets?${params.toString()}`;
...
export const fetchCustomerPeaks = async (names: string[] = [], salesmanIds: number[], viewType: 'customer' | 'area' | 'storeType' = 'customer', storeType?: string): Promise<Record<string, { total: number; peak: number; metadata?: Record<string, unknown> }>> => {
    let url = `/api/bia/crm/target-setting-reports/divisional-channel/managerial-channel/customer-peak?ids=${salesmanIds.join(",")}${names.length > 0 ? `&names=${encodeURIComponent(names.join("|"))}` : ""}&viewType=${viewType}`;
...
export const fetchCustomerTargets = async (salesmanIds: number[], startDate: string, endDate: string, viewType: 'customer' | 'area' | 'storeType' = 'customer', names: string[] = []): Promise<Record<string, number>> => {
    try {
        const url = `/api/bia/crm/target-setting-reports/divisional-channel/managerial-channel/customer-targets?salesmanIds=${salesmanIds.join(",")}&startDate=${startDate}&endDate=${endDate}&viewType=${viewType}${names.length > 0 ? `&names=${encodeURIComponent(names.join("|"))}` : ""}`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok) return {};
...
export const fetchCustomerProducts = async (
    ...
    const url = `/api/bia/crm/target-setting-reports/divisional-channel/managerial-channel/customer-products?${params.toString()}`;
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) return [];
    return await res.json();
};
