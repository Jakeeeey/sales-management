...
/**
 * Fetches the annual report v2 dashboard data (summary + monthly aggregates).
...
export async function fetchDashboardData(
  ...
  const url = `/api/bia/crm/target-setting-reports/divisional/annual-report-v2${query ? `?${query}` : ""}`;
  ...
  if (!response.ok) {
    throw new Error(
      `ERR_API_FAIL: Failed to fetch annual report v2 data: ${response.statusText}`,
    );
  }
