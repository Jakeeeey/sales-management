...
/**
 * Custom hook for fetching and managing annual report v2 dashboard state.
 ...
 */
export function useAnnualReportV2(params: Record<string, string> = {}) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (controller.signal.aborted) return;
      setError(
        err instanceof Error ? err.message : "ERR_INTERNAL_FAIL: Failed to load annual report v2 data",
      );
    } finally {
