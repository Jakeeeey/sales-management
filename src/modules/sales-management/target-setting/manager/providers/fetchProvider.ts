...
export async function bootstrapManagerTargets(): Promise<ManagerBootstrapResponse> {
  return api<ManagerBootstrapResponse>("/api/sales-management/target-setting/manager");
}
...
export async function deleteSupplierAllocation(id: number): Promise<{ ok: true }> {
  await api(`/api/sales-management/target-setting/manager?id=${encodeURIComponent(String(id))}`, {
    method: "DELETE",
  });
  return { ok: true };
}
