...
const API_BASE = "/api/sales-management/target-setting/executive";
...
export async function getTestUser(): Promise<number> {
  ...
  const res = await fetch(`/api/sales-management/target-setting/metadata/users?${params.toString()}`, {
    headers: { "Content-Type": "application/json" },
  });
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  const json = await res.json();
...
export async function getDivisions(): Promise<Division[]> {
  const res = await fetch("/api/sales-management/target-setting/metadata/divisions", {
    headers: { "Content-Type": "application/json" },
  });
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  const json = await res.json();
...
export async function getSuppliers(all = false): Promise<Record<string, unknown>[]> {
  const url = all ? "/api/sales-management/target-setting/metadata/suppliers?all=true" : "/api/sales-management/target-setting/metadata/suppliers";
...
export async function getSalesmen(): Promise<Record<string, unknown>[]> {
  const res = await fetch("/api/sales-management/target-setting/metadata/salesmen", {
    headers: { "Content-Type": "application/json" },
  });
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  const json = await res.json();
