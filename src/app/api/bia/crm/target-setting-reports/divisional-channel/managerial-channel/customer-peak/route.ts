...
/**
 * GET /api/bia/crm/target-setting-reports/managerial-supplier/customer-peak
 */
export async function GET(req: NextRequest) {
  try {
    ...
    allData.forEach((item: VSalesPerformanceDataDto) => {
        ...
        if (viewType === "area") {
            groupName = `${(itemObj.province as string || "").trim()}, ${(itemObj.city as string || "").trim()}`.replace(/^, |, $/g, "") || "Unknown Area";
            ...
            rawCode = (itemObj.customerCode as string || item.storeName || "Unknown").trim();
